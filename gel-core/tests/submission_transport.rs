...
fn new_submission_payload_strictly_omits_datetime_and_preserves_authenticated_teacher() {
    ...
    assert_eq!(
        payload.get(gel_fields::TEACHER_ID).map(|s| s.as_str()),
        Some("326743")
    );
    assert_eq!(
        payload.get(gel_fields::STUDENT_UID).map(|s| s.as_str()),
        Some("200")
    );
    assert_eq!(
        payload.get(gel_fields::TUTORIAL_TYPE).map(|s| s.as_str()),
        Some("0")
    );
    assert_eq!(
        payload.get(gel_fields::CUSTOM_DATE).map(|s| s.as_str()),
        Some("27-08-2026")
    );
}
...
fn unauthenticated_session_fails_closed_before_network_dispatch() {
    ...
    if let Ok(mut session) = session_res {
        ...
        assert!(matches!(
            res,
            Err(gel_core::submission_transport::SubmissionError::Unauthenticated)
        ));
    }
}
