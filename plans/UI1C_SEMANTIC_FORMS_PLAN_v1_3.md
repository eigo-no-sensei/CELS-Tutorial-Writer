...
## UI1c1 implementation amendment

UI1c1 is implemented by explicit user direction while strict GRI acceptance remains outstanding. This is an implementation authorization only, not a promotion. The common framework provides reusable disposition-aware date/select/checkbox/textarea/read-only controls, consumes Rust-projected option domains and limits, and routes structured `DraftValidationIssue` objects without parsing message text. Only the existing foundation surface is rendered; UI1c2–UI1c4 remain responsible for the complete Standard/Initial/Final sections.
